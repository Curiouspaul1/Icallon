...
from utils import (
    genRoomId, addToRoom, removeFromRoom, get_player_room,
    map_player_to_room, indexRoom, get_player_turn, store_sid,
    remove_sid, get_players, get_sid, is_in_session, set_room_mode,
    get_used_letters, cross_letter,
    get_answer_validity, commit_round_scores, get_room_categories,
    get_room_config, set_turn_player, get_turn_player
)
...
def set_session(data):
    if 'session' in data: session['user_id'] = data['session']
...
def disconnect(reason):
    ...
    if player:
        ...
        if room_id:
            ...
            if remaining: emit('player_left', remaining, to=room_id)
        remove_sid(player)
...
def letter_selected(data):
    ...
    round_states[room_id] = {
        ...
        'contested_items': [], 'votes_cast_count': 0
    }
        ...
        for player, p_answers in state['answers'].items():
            ...
            for category, details in report.items():
                if details['status'] == 'needs_vote':
                    contested.append({
                        'id': str(uuid.uuid4()), 'player': player,
                        'word': details['word'], 'category': category,
